...
int lf = 10; // Newline character

void setup() {
  f = createFont("Noto Serif", 16, true); // Create Font
  img = loadImage("cobra-logo-white.png"); //Load logo image
  size(1000, 800);
...
void draw() {
  ...
  stroke(0);
  fill(50,50,50);
  circle(firstColumn+60, firstRow+60, 70);
  circle(firstColumn+210, firstRow+60, 70);
  circle(firstColumn+360, firstRow+60, 70);
  ...
  text("X axis "+parsed_values[2], firstColumn, firstRow);
  text("Y axis "+parsed_values[3], firstColumn+columnGap, firstRow);
  text("Z axis "+parsed_values[4], firstColumn+columnGap*2, firstRow);
  
  float x_axis = radians(float(parsed_values[2]));
  float y_axis = radians(float(parsed_values[3]));
  float z_axis = radians(float(parsed_values[4]));
